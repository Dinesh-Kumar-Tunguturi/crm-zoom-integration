...
export default function SalesAddonsInfo() {
  ...
  const fetchPRData = async (profile: Profile) => {
    try {
      ...
      (rp || []).forEach((r) => {
        let paths: string[] = [];
        if (Array.isArray(r.pdf_path)) {
          paths = r.pdf_path;
        } else if (typeof r.pdf_path === "string") {
          const trimmed = r.pdf_path.trim();
        ...
        rpMap.set(r.lead_id, { status: r.status, pdf_path: paths });
      });
              ...
              {prLoading ? (
                ...
                pagedRows.map((r, i) => (
                  <TableRow key={r.lead_id}>
                    ...
                    <TableCell className="max-w-28 overflow-hidden text-center">
                      {r.resumePdf && r.resumePdf.length > 0 ? (
                        r.resumePdf.length === 1 ? (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => downloadResume(r.resumePdf[0])}
                            className="h-6"
                          >
                            <p className="text-[10px]">Download ({r.resumePdf.length})</p>
                          </Button>
                        ) : (
                          <Dialog>
                            ...
                            <DialogContent>
                              <DialogHeader>
                                <DialogTitle>Select Resume to Download</DialogTitle>
                              </DialogHeader>
                              <div className="space-y-2 mt-2">
                                {r.resumePdf.map((path, idx) => (
                                  <div
                                    key={idx}
                                    className="flex items-center justify-between p-2 border rounded hover:bg-gray-50 cursor-pointer"
                                    onClick={() => downloadResume(path)}
                                  >
                                    <span className="text-sm truncate max-w-[300px]" title={path}>
                                      {path.split("/").pop()}
                                    </span>
