...
export default function SalesPage() {
  ...
  //   // Flatten each record (show only latest follow-up if multiple)
  ...
  //     assigned_to: lead.assigned_to,
  //     current_stage: lead.current_stage,
  //     followup_date: lead.call_history?.[0]?.followup_date ?? "N/A",
  //     notes: lead.call_history?.[0]?.notes ?? "N/A",
  //   }));
  // };


  const fetchFollowUps = async () => {
    // Step 1: Get all leads where stage is DNP or Conversation Done
    ...
    // Step 2: Get call history entries for those leads
