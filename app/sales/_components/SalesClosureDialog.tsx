        ...
        try {
            ...
            if (reportMode === "first") {
                ...
                if (startDate && endDate) {
                    ...
                    if (dateErr) throw dateErr;
                    const inRangeIds = new Set(salesInDate?.map((s) => s.lead_id).filter(Boolean) || []);


                    if (inRangeIds.size === 0) {
                        setData([]); setTotalRecords(0); setTotalRevenue(0); setLoading(false); return;
                    }
                ...
                if (searchTerm) {
                    ...
                    query = query.or(`name.ilike.%${escaped}%,email.ilike.%${escaped}%,business_id.ilike.%${escaped}%,phone.ilike.%${escaped}%`);
                }
                if (sourceFilter && sourceFilter !== "all") query = query.eq("source", sourceFilter);
                if (assignedToFilter && assignedToFilter !== "all") {
                    query = query.eq("assigned_to", assignedToFilter);
                ...
                if (!leadsData || leadsData.length === 0) {
                    setData([]); setTotalRevenue(0); setLoading(false); return;
                }


                // --- PHASE 3: Fetch Oldest Sales Record ---
                const businessIds = leadsData.map(l => l.business_id).filter(Boolean) as string[];
                const { data: salesData, error: salesErr } = await supabase
                    .from("sales_closure")
                    .select("*")
                    .in("lead_id", businessIds);
                ...
                const merged: SaleClosureRow[] = leadsData.map(lead => {
                    const sale = salesMap.get(lead.business_id);
                    return {
                        id: sale?.id || `missing-${lead.business_id}`,
                        lead_id: lead.business_id,
                        lead_name: lead.name,
                        email: lead.email,
                        phone: lead.phone || "-",
                ...
                // --- REVENUE CALCULATION FOR FIRST SALES ---
                ...
                if (dateFilteredIds !== null) allLeadsQuery = allLeadsQuery.in("business_id", dateFilteredIds);
                if (searchTerm) {
                    const escaped = searchTerm.replace(/[,()]/g, '');
                    allLeadsQuery = allLeadsQuery.or(`name.ilike.%${escaped}%,email.ilike.%${escaped}%,business_id.ilike.%${escaped}%,phone.ilike.%${escaped}%`);
                }
                if (sourceFilter && sourceFilter !== "all") allLeadsQuery = allLeadsQuery.eq("source", sourceFilter);
                if (assignedToFilter && assignedToFilter !== "all") {
                    allLeadsQuery = allLeadsQuery.eq("assigned_to", assignedToFilter);
                }


                const { data: allMatchingLeads, error: amtErr } = await allLeadsQuery;
                if (amtErr) throw amtErr;


                if (allMatchingLeads && allMatchingLeads.length > 0) {
                    const allLeadIds = allMatchingLeads.map(l => l.business_id).filter(Boolean) as string[];
                    if (allLeadIds.length > 0) {
                        const { data: allSalesData, error: sumSalesErr } = await supabase
                            .from("sales_closure")
                            .select("lead_id, sale_value, closed_at")
                            .in("lead_id", allLeadIds);
                        if (sumSalesErr) throw sumSalesErr;


                        if (allSalesData) {
                            const tempMap = new Map<string, any>();
                            allSalesData.forEach(sale => {
                                const existing = tempMap.get(sale.lead_id);
                                if (!existing || dayjs(sale.closed_at).isBefore(dayjs(existing.closed_at))) {
                                    tempMap.set(sale.lead_id, sale);
                                }
                            });
                            let revenueSum = 0;
                            tempMap.forEach(s => revenueSum += (s.sale_value || 0));
            ...
            // MODE B: "ONLY RENEWALS"
                ...
                const { data: allSales, error: allErr } = await supabase
                    .from("sales_closure")
                    .select("id, lead_id, closed_at, sale_value");
                ...
                // --- FILTER BY SEARCH / SOURCE / ASSIGNED TO (for Renewals) ---
                if (searchTerm || (sourceFilter && sourceFilter !== "all") || (assignedToFilter && assignedToFilter !== "all")) {
                    let lq = supabase.from("leads").select("business_id");
                    if (searchTerm) {
                        const escaped = searchTerm.replace(/[,()]/g, '');
                        lq = lq.or(`name.ilike.%${escaped}%,email.ilike.%${escaped}%,business_id.ilike.%${escaped}%,phone.ilike.%${escaped}%`);
                    }
                    if (sourceFilter && sourceFilter !== "all") lq = lq.eq("source", sourceFilter);
                    if (assignedToFilter && assignedToFilter !== "all") lq = lq.eq("assigned_to", assignedToFilter);


                    const { data: matchingLeads, error: matchErr } = await lq;
                    if (matchErr) throw matchErr;


                    const validLeadIds = new Set(matchingLeads?.map(l => l.business_id).filter(Boolean));


                    const saleToLeadMap = new Map(allSales?.map(s => [s.id, s.lead_id]));
                    renewalSaleIds = renewalSaleIds.filter(id => {
                        const lid = saleToLeadMap.get(id);
                        return lid && validLeadIds.has(lid);
                    });
                }


                // --- REVENUE CALCULATION FOR RENEWALS ---
                const salesMapForRen = new Map(allSales?.map(s => [s.id, s.sale_value]));
                let renSum = 0;
                renewalSaleIds.forEach(id => {
                    renSum += (salesMapForRen.get(id) || 0);
                });
                setTotalRevenue(renSum);
                ...
                const distinctLeadIds = Array.from(new Set(salesDetails?.map(s => s.lead_id).filter(Boolean)));
                ...
                // --- BROAD SEARCH FOR ALL SALES ---
                let searchLeadIds: string[] | null = null;
                if (searchTerm) {
                    const escaped = searchTerm.replace(/[,()]/g, '');
                    const { data: matchingLeads, error: matchErr } = await supabase
                        .from("leads")
                        .select("business_id")
                        .or(`name.ilike.%${escaped}%,email.ilike.%${escaped}%,business_id.ilike.%${escaped}%,phone.ilike.%${escaped}%`);


                    if (matchErr) throw matchErr;
                    searchLeadIds = (matchingLeads?.map(l => l.business_id) || []).filter(Boolean) as string[];
                ...
                if (searchLeadIds !== null && searchLeadIds.length > 0) {
                ...
                const leadIds = Array.from(new Set(salesData.map(s => s.lead_id).filter(Boolean)));
                if (leadIds.length === 0) {
                    setData([]); setLoading(false); return;
                }
                const { data: leadsInfo, error: leadsErr } = await supabase
                ...
                // --- REVENUE CALCULATION FOR ALL SALES ---
                let sumQuery = supabase.from("sales_closure").select("sale_value, lead_id");
                if (startDate && endDate) {
                    sumQuery = sumQuery.gte("closed_at", dayjs(startDate).startOf("day").toISOString())
                        .lte("closed_at", dayjs(endDate).endOf("day").toISOString());
                }


                if (searchLeadIds !== null) {
                    sumQuery = sumQuery.in("lead_id", searchLeadIds);
                }
                ...
                if (sumData) {
                    if ((sourceFilter && sourceFilter !== "all") || (assignedToFilter && assignedToFilter !== "all")) {
                        let lq = supabase.from("leads").select("business_id");
                        if (sourceFilter && sourceFilter !== "all") lq = lq.eq("source", sourceFilter);
                        if (assignedToFilter && assignedToFilter !== "all") lq = lq.eq("assigned_to", assignedToFilter);


                        const { data: matchingLeads, error: leadsFilterErr } = await lq;
                        ...
                        const filteredSumData = sumData.filter(s => validLeadIds.has(s.lead_id));
                        const total = filteredSumData.reduce((acc, curr) => acc + (curr.sale_value || 0), 0);
                        setTotalRevenue(total);
                    } else {
                        const total = sumData.reduce((acc, curr) => acc + (curr.sale_value || 0), 0);
                        setTotalRevenue(total);
                    }
                } else {
                    setTotalRevenue(0);
    ...
    const handleExport = async () => {
        try {
            ...
            if (reportMode === "first") {
                ...
                const businessIds = allLeads.map(l => l.business_id).filter(Boolean) as string[];
                const { data: salesData, error: salesErr } = await supabase
                    .from("sales_closure")
                    .select("*")
                    .in("lead_id", businessIds);
            ...
            // MODE B: "ONLY RENEWALS"
                ...
                if (searchTerm || (sourceFilter && sourceFilter !== "all") || (assignedToFilter && assignedToFilter !== "all")) {
                    ...
                    const { data: matchingLeads, error: leadsErr } = await lq;
                    if (leadsErr) throw leadsErr;
                ...
                const distinctLeadIds = Array.from(new Set(salesDetails?.map(s => s.lead_id).filter(Boolean)));
                if (distinctLeadIds.length === 0) { alert("No renewal data found"); return; }
            ...
            // MODE C: "ALL SALES"
                ...
                if (searchTerm) {
                    ...
                    const searchLeadIds = (matchingLeads?.map(l => l.business_id) || []).filter(Boolean) as string[];
                    if (searchLeadIds.length === 0) {
                ...
                const leadIds = Array.from(new Set(salesData.map(s => s.lead_id).filter(Boolean)));
                if (leadIds.length === 0) { alert("No data to export"); return; }
        ...
        <Dialog open={isOpen} onOpenChange={(v) => !v && onClose()}>
            <DialogContent className="w-[95vw] max-w-[95vw] h-[95vh] flex flex-col">
                <DialogHeader>
                    ...
                    {/* Report Mode & Revenue Overview */}
                    <div className="flex flex-col sm:flex-row items-center gap-6 py-4">
                        {/* Revenue Card (Left Side) */}
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3 px-5 shadow-sm flex flex-col items-center justify-center min-w-[200px]">
                            <span className="text-xs text-green-600 font-semibold uppercase tracking-wide">Total Revenue</span>
                            <span className="text-2xl font-bold text-green-700 mt-0.5">
                ...
                <div className="flex items-center justify-between py-2 border-t mt-auto px-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={() => setPage(p => p + 1)}
                            disabled={page * (pageSize === 999999 ? totalRecords : pageSize) >= totalRecords}
                        >
                            Next
                        </Button>
                    </div>
