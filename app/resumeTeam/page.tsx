...
//   </DropdownMenuTrigger>
//   <DropdownMenuContent align="end">
//     {/* Not Onboarded Clients Link */}
//     <DropdownMenuItem>
//        <Link href="/clients/not-onboarded" passHref>
...
//     {/* Only for Resumes Link */}
//     <DropdownMenuItem>
//       <Link href="/clients/resume-only" passHref>
//         <a target="_blank" rel="noopener noreferrer">
//           Only for Resumes
//         </a>
//       </Link>
...
//     {/* Job Board Clients Link */}
//     <DropdownMenuItem>
//        <Link href="/clients/job-board" passHref>
//         <a target="_blank" rel="noopener noreferrer">
//           Job Board Clients
//         </a>
//       </Link>
...
// // import  RefreshStatusDialog  from "./components/RefreshStatusDialog";  // Import the dialog
// // import  DateRangeFilter  from "./components/DateRangeFilter";
// // import  AssigneeFilter  from "./components/AssigneeFilter";

// // const ResumeTeamPage = () => {
...
// //   const [searchTerm, setSearchTerm] = useState("");
// //   const [selectedTab, setSelectedTab] = useState("resume"); // Default to "resume"
  
// //   const { rows, fetchData } = useFetchData();
...
// //     const query = searchTerm.toLowerCase().trim();
// //     return (
// //       r.lead_id.toLowerCase().includes(query) ||
// //       r.leads?.name.toLowerCase().includes(query) ||
...
// //   return (
// //     <div className="p-6">
// //       <h1 className="text-3xl font-semibold text-gray-900">Resume Team Dashboard</h1>
...
// //           startDate={""}
// //           endDate={""}
// //           onStartDateChange={(value) => console.log(value)} // Implement date filtering
// //           onEndDateChange={(value) => console.log(value)} // Implement date filtering
// //           onApply={() => fetchData()}
// //           onClear={() => fetchData()}
// //         />
// //         <AssigneeFilter
// //           assigneeFilter="__all__"
// //           resumeTeamMembers={[]} // Pass the team members here
// //           onFilterChange={(assigneeEmail) => fetchData({ assigneeEmail })}
// //         />
...
// //       {/* Table Component */}
// //       <ResumeTable rows={filteredRows} loading={loading} fetchData={fetchData} />

// //       {/* Refresh Status Dialog */}
// //       <RefreshStatusDialog open={dialogOpen} status={status} onClose={() => setDialogOpen(false)} />
// //     </div>
// //   );
// // };

// // export default ResumeTeamPage; // Ensure default export is included here
...
const [filterDialogOpen, setFilterDialogOpen] = useState(false);
// const [filterTab, setFilterTab] = useState<"notOnboarded" | "resumeOnly">("notOnboarded");
const [filterTab, setFilterTab] = useState<"notOnboarded" | "resumeOnly" | "jobBoardClients">("notOnboarded");
...
const [notOnboardedDialogOpen, setNotOnboardedDialogOpen] = useState(false);
const [resumeOnlyDialogOpen, setResumeOnlyDialogOpen] = useState(false);
const [jobBoardClientsDialogOpen, setJobBoardClientsDialogOpen] = useState(false);
...
const handleOnboardClickForJobBoard = async (row: SalesClosure) => {
  // You can use the same function as the main table
  // Ensure you're passing the correct context (Job Board Client)
  await handleOnboardClick(row);  // Reusing the existing function
};
...
const fetchFilteredClients = async (mode: "notOnboarded" | "resumeOnly" | "jobBoardClients") => {
  try {
    ...
    let query = supabase
      .from("sales_closure")
      .select(
        "id, lead_id, email, finance_status, closed_at, resume_sale_value, application_sale_value, portfolio_sale_value, commitments, data_sent_to_customer_dashboard, company_application_email, onboarded_date, badge_value, job_board_value"
      )
    ...
    if (mode === "notOnboarded") {
      ...
      query = query.gt("job_board_value", 0);
    }
...
const handleSendToPendingClientsForJobBoard = async (leadId: string) => {
  try {
    console.log("Sending Job Board Client data to Pending Clients...");
    await handleSendToPendingClients(leadId);  // Reuse the existing function here
    alert("✅ Data successfully sent to pending_clients.");
    await fetchData(); // Refresh the dashboard
  } catch (err: any) {
    console.error("Error:", err.message);
    alert("❌ Failed to send data: " + err.message);
  }
...
{/* 
    ...
    <DropdownMenuItem
      onClick={async () => {
        ...
        const data = await fetchFilteredClients("jobBoardClients");
        setFilterRows(data);
        setJobBoardClientsDialogOpen(true);  // Open dialog for Job Board Clients
        setFilterLoading(false);
      }}
    >
      Job Board Clients
...
<Dialog open={resumeOnlyDialogOpen} onOpenChange={setResumeOnlyDialogOpen}>
  <DialogContent className="max-w-[90vw] max-h-[80vh] overflow-auto">
    <DialogHeader>
      <DialogTitle>Only for Resumes</DialogTitle>
      <DialogDescription>Clients who paid for resumes but not for applications</DialogDescription>
    </DialogHeader>
    <div className="overflow-x-auto">
      {filterLoading ? (
        <div className="p-6 text-sm text-muted-foreground">Loading…</div>
...
{/* Job Board Clients Dialog */}
<Dialog open={jobBoardClientsDialogOpen} onOpenChange={setJobBoardClientsDialogOpen}>
  <DialogContent className="max-w-[90vw] max-h-[80vh] overflow-auto">
    <DialogHeader>
      <DialogTitle>Job Board Clients</DialogTitle>
      <DialogDescription>Clients who paid for job board value</DialogDescription>
    </DialogHeader>
    <div className="overflow-x-auto">
      {filterLoading ? (
        <div className="p-6 text-sm text-muted-foreground">Loading…</div>
      ) : (
        renderTable(filterRows, "jobBoardClients")
      )}
    </div>
    <DialogFooter>
      <Button onClick={() => setJobBoardClientsDialogOpen(false)}>Close</Button>
    </DialogFooter>
  </DialogContent>
</Dialog>
