...
export default function ApplicationsPage() {
  ...
  const fetchData = async (
    ...
    try {
      ...
      // Apply assignee filter (only if not showing "My Tasks" and not "All")
      else if (activeAssigneeFilter !== "__all__" && activeAssigneeFilter !== "") {
        query = query.eq("resume_assigned_email", activeAssigneeFilter);
      }
  ...
  const onChangeStatus = async (row: SalesClosure, newStatus: ResumeStatus) => {
    try {
      ...
      await fetchData(page, limit, searchQuery, assigneeFilter, showMyTasks);
    } catch (e: any) {
      alert(e.message || "Failed to update status");
    }
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={async (val) => {
              const newPage = Number(val);
              setPage(newPage);
              await fetchData(newPage, limit, searchQuery, assigneeFilter, showMyTasks);
            }}
          >
            <SelectTrigger className="w-[100px]">
              <SelectValue />
          ...
          <div className="flex items-center gap-3">
            ...
            <Select
              ...
              onValueChange={async (val) => {
                ...
                await fetchData(1, limit, searchQuery, val, showMyTasks);
              }}
            >
              <SelectTrigger className="w-[260px]">
                <SelectValue placeholder="All team members" />
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                ...
                onValueChange={async (val) => {
                  ...
                  setLimit(newLimit);
                  setPage(1); // reset page
                  await fetchData(1, newLimit, searchQuery, assigneeFilter, showMyTasks);
                }}
              >
                <SelectTrigger className="w-[80px]">
                  <SelectValue />
          ...
          <div className="flex items-center gap-3 w-full max-w-lg">
            <Input
              ...
              onKeyDown={async (e) => {
                if (e.key === "Enter") {
                  setSearchQuery(searchText);
                  setPage(1);
                  await fetchData(1, limit, searchText, assigneeFilter, showMyTasks);
                }
              }}
            />
            <Button
            ...
            <Button
              ...
              onClick={async () => {
                ...
                setShowMyTasks(newValue);
                setPage(1);
                await fetchData(1, limit, searchQuery, assigneeFilter, newValue);
              }}
            >
              {showMyTasks ? "Show All" : "My Tasks"}
            </Button>
