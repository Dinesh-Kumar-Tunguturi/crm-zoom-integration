...
// /**
//  * TotalRevenueCardPage
//  * - Minimal page that shows Total Collected from `sales_closure` with an IST-aware date range selector.
//  * - Assumes `closed_at` values in the DB are IST-local timestamps (e.g. '2025-10-01T10:30:00' or '2025-10-01 10:30:00').
//  */
// export default function TotalRevenueCardPage(): JSX.Element {
...
//   // Produce IST-local datetime strings WITHOUT timezone (suitable for comparing with timestamp without time zone)
//   // Returns strings in `YYYY-MM-DDTHH:mm:ss` form (no trailing 'Z').
//   const buildIstLocalBounds = (start?: string, end?: string): { startIst?: string; endIst?: string } => {
//     if (!start || !end) return { startIst: undefined, endIst: undefined };

//     // sanity: ensure format YYYY-MM-DD
//     const valid = (s: string) => /^\d{4}-\d{2}-\d{2}$/.test(s);
//     if (!valid(start) || !valid(end)) return { startIst: undefined, endIst: undefined };

//     // IST start-of-day and end-of-day (local) — no timezone suffix
//     const startIst = `${start}T00:00:00`;
//     const endIst = `${end}T23:59:59`;
...
//     try {
//       const projection = 'sale_value, closed_at';
//       // If both dates are provided, convert them to IST-local strings and query using those
//       if (startDate && endDate) {
//         const { startIst, endIst } = buildIstLocalBounds(startDate, endDate);
//         if (!startIst || !endIst) throw new Error('Invalid date format. Use YYYY-MM-DD');

//         // Debug: show bounds
//         // console.log('Querying IST local bounds', { startIst, endIst });

//         // Query rows where closed_at (stored as IST-local) is between these bounds.
//         // We pass the IST-local strings directly so Postgres compares them as timestamp without timezone.
//         const { data, error } = await supabase
//           .from('sales_closure')
//           .select(projection)
//           .gte('closed_at', startIst)
//           .lte('closed_at', endIst);

//         if (error) throw error;
//         const sum = (data ?? []).reduce((acc: number, r: any) => acc + (Number(r.sale_value) || 0), 0);
//         setTotalCollected(Number(sum.toFixed(2)));
//       } else {
//         // No range -> sum entire table
//         const { data, error } = await supabase.from('sales_closure').select(projection);
//         if (error) throw error;
//         const sum = (data ?? []).reduce((acc: number, r: any) => acc + (Number(r.sale_value) || 0), 0);
//         setTotalCollected(Number(sum.toFixed(2)));
//       }
//     } catch (err: any) {
//       console.error('fetchTotal failed', err);
...
//             <Button onClick={() => fetchTotal()}>Apply</Button>
//             <Button
//               variant="ghost"
//               onClick={() => {
//                 setStartDate('');
...
//               {isLoading || totalCollected === null ? 'Loading…' : formatCurrency(totalCollected)}
//             </p>
//             <p className="text-sm text-muted-foreground mt-2">
//               {startDate && endDate ? `Range (IST): ${startDate} → ${endDate}` : 'Range: Full table'}
//             </p>
...
const fetchAll = async () => {
  ...
  try {
    ...
    // ---- helpers for UTC day boundaries ----
const MS_PER_DAY = 1000 * 60 * 60 * 24;
    const today = new Date();

const toUtcStartOfDay = (d: Date) =>
  Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()); // ms at 00:00:00 UTC

const toUtcEndOfDayExclusive = (d: Date) =>
  Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()) + MS_PER_DAY; // exclusive end

// parseLocalDate as used earlier (keeps), but convert any Date -> use UTC-day helpers below
const parseLocalDateToDate = (s?: string | null): Date | undefined => {
  if (!s) return undefined;
  // if string contains time info (ISO) the Date constructor will parse it (possibly with offset)
  // If it's a plain 'YYYY-MM-DD' we'll construct as local midnight; still we'll convert to UTC-day below.
  return s.includes('T') ? new Date(s) : new Date(`${s}T00:00:00`);
};

// Compute usable revenue by intersecting subscription window with selected window (or up to today)
const usableNum = rowsInRange.reduce((acc, r) => {
  const cycleDays = safeCycleDays(r.subscription_cycle, 30);
  const totalAmount = safeNumber(r.sale_value, 0);
  if (totalAmount === 0 || cycleDays <= 0) return acc;

  // subscription start (use onboarded_date if present, otherwise closed_at)
  const subscriptionDate = parseLocalDateToDate(r.onboarded_date ?? r.closed_at) ?? parseLocalDateToDate(r.closed_at)!;
  // subscription period in UTC-day coordinates: [subStartUtc, subEndUtc) exclusive-end
  const subStartUtcMs = toUtcStartOfDay(subscriptionDate);
  const subEndUtcMs = subStartUtcMs + cycleDays * MS_PER_DAY; // exclusive

  // window to measure usable revenue inside:
  // if user selected date range, use [rangeStartUTC, rangeEndUTC] (rangeEndUTC was produced with T23:59:59.999 and is in UTC)
  // we will convert them to UTC-day exclusive-end
  const windowStartUtcMs = hasDateRange && rangeStartUTC
    ? toUtcStartOfDay(new Date(rangeStartUTC))
    : subStartUtcMs; // if no date range then window start = subscription start

  const chosenRangeEnd = hasDateRange && rangeEndUTC ? new Date(rangeEndUTC) : today;
  const windowEndUtcMsCandidate = toUtcEndOfDayExclusive(chosenRangeEnd); // exclusive end
  // never extend window beyond today (use today exclusive end)
  const todayEndUtcMs = toUtcEndOfDayExclusive(today);
  const windowEndUtcMs = Math.min(windowEndUtcMsCandidate, todayEndUtcMs);

  // intersection of [subStartUtcMs, subEndUtcMs) and [windowStartUtcMs, windowEndUtcMs)
  const interStart = Math.max(subStartUtcMs, windowStartUtcMs);
  const interEnd = Math.min(subEndUtcMs, windowEndUtcMs);

  if (interEnd <= interStart) return acc; // no overlap

  const daysInWindow = (interEnd - interStart) / MS_PER_DAY; // may be fractional if times not aligned, but using day boundaries keeps it integral
  const perDay = totalAmount / cycleDays;
  const usableForRow = perDay * daysInWindow;
  return acc + (Number.isFinite(usableForRow) ? usableForRow : 0);
}, 0);
